...
def create_fid_backbone(fid_classifier) -> nn.Module:
    ...
    fid_backbone = nn.Sequential(convs, clf_no_head)
    return fid_backbone


def train(
    model, trainloader, valloader, num_epochs, lr, val_acc_every, device=None
) -> None:
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        for epoch in tqdm(range(num_epochs), desc="Training"):
            ...
            mlflow.log_metric(
                "Cross_entropy_epoch", running_loss / len(trainloader), step=epoch
            )
